...
from utils.datasets_dev import LoadStreams, LoadImages
...
def detect(save_img=False):
    ...
    if webcam:
        ...
        save_img = True
        niche = True # Pass a Niche flag to handle Columbrium folders
...
if __name__ == '__main__':
    ...
    parser.add_argument('--weights', nargs='+', type=str, default=r'C:\YOLOv5\yolov5\weights\niche.pt', help='model.pt path(s)') # defaults to trained weights
    ...
    files = []
    all_files = sorted(Path(img_path).rglob('*.*')) # Search from input path
    for x in all_files:
        if re.match('Snagged_TO BE CROPPED', x.parts[len(x.parts) - 3], re.IGNORECASE):
            files.append(Path(x))

    # Only take image files we want
    img_formats = ['.jpg', '.jpeg']
    origList = [x for x in files if os.path.splitext(x)[-1].lower() in img_formats]
    #origList = list(img_path.glob( '*.jpg' )) # Original version
    ...
    for file in tqdm.tqdm(detectList, desc="Cropping, Resizing & Saving Images"):
        # Find matching image in list
        txt_name = file.stem
        for imagePath in origList:
            if re.match(txt_name, imagePath.stem):
                niche = imagePath
        ...
        with file.open() as f:
            ...
            x_start = x_cen - int(width / 2) - padding - side_padding
            x_end = x_cen + int(width / 2) + padding + side_padding

            y_start = y_cen - int(height / 2) - padding - top_padding
...
# C:\YOLOv5\yolov5\Scripts1\python.exe C:\YOLOv5\yolov5\detect.py --source 'Section0'